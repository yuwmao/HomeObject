...
PGManager::NullAsyncResult HomeObjectImpl::start_replace_member(pg_id_t id, peer_id_t const& old_member,
                                                                ...
                                                                trace_id_t tid) {
    LOGI("[pg={}] start replace member [{}] with [{}] quorum [{}] trace_id [{}]",
...
PGManager::NullAsyncResult HomeObjectImpl::complete_replace_member(pg_id_t id, peer_id_t const& old_member,
                                                                   PGMember& new_member, uint32_t commit_quorum,
                                                                   trace_id_t tid) {
    LOGI("[pg={}] complete replace member [{}] with [{}] quorum [{}] trace_id [{}]", id, to_string(old_member),
         to_string(new_member.id), commit_quorum, tid);
